...
The current parser repair is a local commit on `fix-current-source-schemas`;
build it from that Funes worktree until it is separately published. For published
revisions, fetch the maintained `audiodude/funes` fork and check out the exact pin
before building. A branch name or previously installed executable does not
establish that its build matches this revision.
