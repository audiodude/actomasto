...
Status: implemented and verified in isolated worktrees and a synthetic sandbox, following separate authorization for the maintained fork, companion PRs, and a synthetic provider run capped at $1. This does not authorize live installation, real-source enrollment, migration of live state, deployment, or transmission of real conversations. Prepared with AI assistance from OpenAI Codex.

The [milestone 1 specification](mastodon-activity-app-notes.md) and [README](README.md) now describe the Funes-based conversation-ingestion architecture. The acceptance evidence below concerns this integration; historical milestone results are not substituted for current verification.
...
- Fork regressions: 272 library tests, one CLI test, and 15 normalization tests, including late-page reachability beyond former cumulative limits.
- Hosted acceptance: eight synthetic drafts, $0.010623 recorded spend, zero outstanding reservation, and a fully caught-up export. The example preserves `assistant_reported` provenance. Git reached accepted settlement while all three conversation harnesses were unavailable; an old-session append and late backfill settled after independent refresh and restart.

| Acceptance area | Current evidence |
| --- | --- |
| Three-harness equivalence | 32 differential cases against the original adapters, plus real CLI fixture contracts |
| Human provenance | Native attribution/confirmation and Unicode framing regressions; hosted evidence retains provenance |
| Completion and branches | Fork normalization and consumer completion, shared-ancestor, partial-write, and future-time cases |
| Authorization boundaries | Interval/nested-repository cases and scope-revocation checks at read, filter, dispatch, and settlement |
| Complete filtering | Whole-original-turn runtime test plus policy/generation secret and blocklist regressions |
| Incremental coverage | Stable snapshot/cursor tests, large late-page case, actual append and backfill recovery |
| Source mutation | Revision-bound rewrite, rotation, truncation, disappearance, and missing-root recovery cases |
| Exact migration | Preserved lifecycle-table comparisons, legacy fallback/adapter-only marker cases, actual CLI reopen proof |
| Failure isolation | Harness-specific schema tests and actual Git generation during the whole-Funes outage |
| Dispatch and lifecycle | Existing off/revoke/purge regressions and added dependency/scope race tests |
| Local-only operation | Actual remote-selector rejection, metadata-only capabilities, independent three-harness refresh |
| Fork and bridge | Fresh-checkout pinned build, stale-build rejection, indexing lifecycle and native MCP smoke checks |
| Real generation path | Actual fork → daemon → Anthropic → SQLite/evidence/export, synthetic only |

Limits: no live installation, enrollment, deployment, or posting; the daemon smoke used an isolated loginctl shim rather than real logind integration. The bridge's five-session run does not renew its historical large-load or spontaneous-model-recall claims. Local semantic indexing reported the scanner unavailable and is not described as sanitized. Smoke daemons were stopped and the temporary provider credential removed.
